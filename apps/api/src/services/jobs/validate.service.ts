import { Placeholder } from "#app/types/placeholder-types";
import { CreateJobParams, Entry, Semantics, ValidationIssueEntry, DeliveryContent, Delivery } from "../../types/jobs-types";

export function validate(recipients: CreateJobParams["recipients"], semantics: CreateJobParams["semantics"], placeholders: Placeholder[]) {
    const sanitizedData: Entry[] = [];

    const placeholderFields = new Set(placeholders.map((p) => p.key));
    
    const deliveryFields = new Map<string, Delivery>();
    ...
    const foundFields = recipients.length > 0 ? Object.keys(recipients[0]) : [];
